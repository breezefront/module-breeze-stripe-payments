...
define(
    ...
    function (
        ...
        return {
            component: "StripeIntegration_Payments/js/stripe_payments_express", // Breeze fix: added component
            shippingAddress: [],
            shippingMethod: null,
            ...
            getApplePayParams: function(type, callback)
            {
                ...
                return $.ajax({ // Breeze fix: storage.post => jQuery.ajax
                    url: serviceUrl,
                    type: 'POST',
                    data: JSON.stringify(payload),
                    global: false,
                    contentType: 'application/json',
                    error: function (xhr, textStatus, errorThrown) { // Breeze fix: fail => error
                        console.error("Could not retrieve initialization params for Apple Pay");
                    },
                    complete: function (response) { // Breeze fix: done => complete
                        if (typeof response.body === 'string') { // Breeze fix: response => response.body
            ...
            addToCart: function(request, shipping_id, callback)
            {
                ...
                return $.ajax({ // Breeze fix: storage.post => jQuery.ajax
                    ...
                    complete: function (response) { // Breeze fix: done => complete
                        $.customerData.invalidate(['cart']); // Breeze fix: customerData => $.customerData
            ...
            processResponseWithPaymentIntent: function(response, callback)
            {
                ...
                {
                    if (typeof response.body === 'string') {
                        response = JSON.parse(response.body); // Breeze fix: response => response.body
                    }
